...
│   ├── csv.ts                  ← Génération CSV (échappement, BOM) + export leads
...
La **fiche lead** (`/leads/[id]`) expose des **actions manuelles** (marquer RDV pris, disqualifier, rouvrir) via `POST /api/leads/[id]/action` — mêmes gardes de périmètre que la lecture (un client n'agit que sur ses leads). Composant client `LeadActions.tsx`. Elle porte aussi des **notes internes** (`leads.notes`, migration 003) éditées via `PATCH /api/leads/[id]/note` (mêmes gardes).

La **liste de leads** est filtrable (statut, catégorie), cherchable (nom/email) et paginée (20/page). Elle est **exportable en CSV** (`GET /api/leads/export`, `src/lib/csv.ts`) : mêmes filtres que la liste, scope client imposé par la session, séparateur `;` + BOM pour Excel FR. Filtres en query params via un formulaire GET server-rendered (URLs partageables) ; logique pure dans `src/lib/leads-filter.ts` (`parseFilters`, `buildPagination`, `sanitizeSearch` — testés), requête dans `getLeadsList`. Le funnel et les compteurs restent calculés sur le scope complet, pas sur le sous-ensemble filtré.
